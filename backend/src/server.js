...
import { advanceRepair, assignRepair, changePassword, confirmDelivery, createAccessorySale, createBanner, createBlogPost, createCategory, createCustomer, createExpense, createInventoryItem, createRepair, createSocialLink, createStaff, createStaffProfile, deactivateStaff, deleteBanner, deleteBlogPost, deleteCategory, deleteExpense, deleteInventoryItem, deleteSocialLink, deleteStaffProfile, getBannerImage, getPublicSiteContent, getStaffPhoto, getWorkspace, login, receiveInventoryStock, recordInvoicePayment, reorderBanner, reorderBlogPost, requestAppointment, requestPasswordReset, resetPassword, reviewAppointment, trackRepair, updateBanner, updateBlogPost, updateCategory, updateCustomer, updateExpense, updateInventoryItem, updateProfile, updateRepairProgress, updateSaleAccounting, updateSocialLink, updateStaff, updateStaffProfile } from './service.js';
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === 'POST' && url.pathname === '/api/sales/accessories') { const session = getSession(requestAdapter(request)); return send(request, response, 201, await createAccessorySale(session.role, session.sub, await readJson(request))); }
    ...
    if (request.method === 'POST' && url.pathname === '/api/inventory/receive') { const session = getSession(requestAdapter(request)); return send(request, response, 201, await receiveInventoryStock(session.role, session.sub, await readJson(request))); }
    ...
    if (request.method === 'POST' && url.pathname === '/api/categories') { const session = getSession(requestAdapter(request)); return send(request, response, 201, await createCategory(session.role, session.sub, await readJson(request))); }
    if (request.method === 'PATCH' && url.pathname === '/api/categories') { const session = getSession(requestAdapter(request)); return send(request, response, 200, await updateCategory(session.role, session.sub, await readJson(request))); }
    if (request.method === 'DELETE' && url.pathname === '/api/categories') { const session = getSession(requestAdapter(request)); return send(request, response, 200, await deleteCategory(session.role, session.sub, (await readJson(request)).id)); }
