...
export default function HomePage() {
  ...
  useEffect(() => { if (workspace?.navigation?.length && active !== 'Settings' && !workspace.navigation.includes(active)) setActive(workspace.navigation[0]); }, [active, workspace?.navigation]);
  ...
  const receiveInventoryStock = async (form) => {
    try { await apiRequest('/api/inventory/receive', token, { method: 'POST', body: JSON.stringify(form) }); await loadWorkspace(); notify('Stock received'); return true; }
    catch (requestError) { setError(requestError.message); return false; }
  };
  ...
  const createCategory = async (form) => {
    try { await apiRequest('/api/categories', token, { method: 'POST', body: JSON.stringify(form) }); await loadWorkspace(); notify('Category added'); return true; }
    catch (requestError) { setError(requestError.message); return false; }
  };
  const updateCategory = async (form) => {
    try { await apiRequest('/api/categories', token, { method: 'PATCH', body: JSON.stringify(form) }); await loadWorkspace(); notify('Category updated'); return true; }
    catch (requestError) { setError(requestError.message); return false; }
  };
  const deleteCategory = async (id, name) => {
    if (!window.confirm(`Delete category "${name}"? This cannot be undone.`)) return;
    try { await apiRequest('/api/categories', token, { method: 'DELETE', body: JSON.stringify({ id }) }); await loadWorkspace(); notify('Category deleted'); }
    catch (requestError) { setError(requestError.message); }
  };
  ...
  const createAccessorySale = async (form) => {
    try { await apiRequest('/api/sales/accessories', token, { method: 'POST', body: JSON.stringify(form) }); await loadWorkspace(); notify('Accessory sale recorded'); return true; }
    catch (requestError) { setError(requestError.message); return false; }
  };
  ...
  const shared = { role, user, repairs, dashboard: workspace?.dashboard || {}, inventory: workspace?.inventory || [], expenses: workspace?.expenses || [], sales: workspace?.sales || [], customers: workspace?.customers || [], team: workspace?.team || [], technicians: workspace?.technicians || [], appointments: workspace?.appointments || [], banners: workspace?.banners || [], socialLinks: workspace?.socialLinks || [], staffProfiles: workspace?.staffProfiles || [], blogPosts: workspace?.blogPosts || [], navigation, permissions, canCreateIntake: navigation.includes('New Intake'), canViewRepairs: navigation.includes('Repairs'), setActive, openIntake };
  const views = { Overview: <Overview {...shared} />, Appointments: <AppointmentsView appointments={shared.appointments} reviewAppointment={reviewAppointment} />, Repairs: <RepairsView repairs={filteredRepairs} totalRepairs={repairs.length} inventory={shared.inventory} technicians={shared.technicians} search={search} setSearch={setSearch} role={role} updateStatus={updateStatus} assignRepair={assignRepair} saveRepairProgress={saveRepairProgress} confirmDelivery={confirmDelivery} />, Inventory: <InventoryView role={role} permissions={shared.permissions} parts={shared.inventory} categories={workspace?.categories || []} dashboard={shared.dashboard} createInventoryItem={createInventoryItem} updateInventoryItem={updateInventoryItem} deleteInventoryItem={deleteInventoryItem} receiveInventoryStock={receiveInventoryStock} createCategory={createCategory} updateCategory={updateCategory} deleteCategory={deleteCategory} />,Expense: <ExpensesView expenses={shared.expenses} dashboard={shared.dashboard} createExpense={createExpense} updateExpense={updateExpense} deleteExpense={deleteExpense} />, 'Point of Sale': <SalesView sales={shared.sales} inventory={shared.inventory} dashboard={shared.dashboard} recordPayment={recordPayment} createAccessorySale={createAccessorySale} />, Customers: <CustomersView role={role} canCreateIntake={shared.canCreateIntake} customers={shared.customers} repairs={repairs} createCustomer={createCustomer} updateCustomer={updateCustomer} startCustomerIntake={openIntake} focusCustomer={focusCustomer} clearFocusCustomer={() => setFocusCustomer(null)} />, Reports: <ReportsView dashboard={shared.dashboard} />, Team: <TeamView team={shared.team} createStaff={createStaff} updateStaff={updateStaff} deactivateStaff={deactivateStaff} />, Website: <WebsiteView role={role} permissions={shared.permissions} banners={shared.banners} socialLinks={shared.socialLinks} staffProfiles={shared.staffProfiles} blogPosts={shared.blogPosts} createBanner={createBanner} updateBanner={updateBanner} deleteBanner={deleteBanner} reorderBanner={reorderBanner} createSocialLink={createSocialLink} updateSocialLink={updateSocialLink} deleteSocialLink={deleteSocialLink} createStaffProfile={createStaffProfile} updateStaffProfile={updateStaffProfile} deleteStaffProfile={deleteStaffProfile} createBlogPost={createBlogPost} updateBlogPost={updateBlogPost} deleteBlogPost={deleteBlogPost} reorderBlogPost={reorderBlogPost} />, Settings: <SettingsView user={user} updateProfile={updateProfile} changePassword={changePassword} emailPasswordReset={emailPasswordReset}/> };
  const currentView = active === 'Settings' || navigation.includes(active) ? views[active] : (views[navigation[0]] || <div className="card list-empty">No sections are available yet. Ask an Admin to grant a permission.</div>);
