...
import { printBoth, printFullReceipt, printSmallTicket } from '../utils/printJobs';
import AccessorySaleForm from './AccessorySaleForm';
...
export function RepairsView({ repairs, totalRepairs = repairs.length, inventory = [], technicians = [], search, setSearch, role, updateStatus, assignRepair, saveRepairProgress, confirmDelivery }) {
  ...
  const [partRows, setPartRows] = useState([{ id: '', quantity: 1 }]);
  ...
  const printTicket = (repair) => {
    sessionStorage.setItem('ifixlab_print_ticket', JSON.stringify({ tickets: [{ ...repair, createdAt: repair.createdAt || new Date().toISOString() }] }));
    window.open('/print-ticket', '_blank');
  };
  const [detailPrintStatus, setDetailPrintStatus] = useState('');
  const runDetailPrint = async (label, action) => {
    setDetailPrintStatus(`${label}…`);
    try { await action(); setDetailPrintStatus(`${label} sent to printer`); }
    catch (error) { setDetailPrintStatus(error.message); }
  };
  ...
  const submitRepairProgress = async (event) => {
    ...
    const parts = partRows.filter((item) => item.id).map((item) => ({ id: item.id, quantity: Number(item.quantity) }));
    const saved = await saveRepairProgress({ id: detailTicket.id, action, notes: data.get('notes'), serviceCharge: serviceChargeValue, ...(action === 'progress' ? { progress: Number(data.get('progress')), parts } : {}) });
  ...
  const historicalPartsCharge = detailTicket?.finalPrice != null ? Math.max(0, Number(detailTicket.finalPrice) - Number(detailTicket.serviceCharge || 0)) : 0;
  const finalCustomerTotal = Number(serviceChargeValue || 0);
  if (role === 'Technician') {
    ...
    return <><PageHead eyebrow="TECHNICIAN WORKSPACE" title="Repair management"><span className="head-count">Received 0% → Diagnosing 25% → Repair Approved 50% → In Repair 75% → Ready for Pickup 100%</span></PageHead><div className="toolbar card list-toolbar"><SearchBox value={search} onChange={setSearch} placeholder="Search ticket, customer, phone, IMEI or device" label="Search repairs"/><ResultCount shown={visibleRepairs.length} total={totalRepairs} noun="repair"/><select value={repairStatusFilter} onChange={(event) => setRepairStatusFilter(event.target.value)} aria-label="Filter repairs by status"><option>All</option>{['Received','Diagnosing','Repair Approved','In Repair','Ready for Pickup','Delivered'].map((status) => <option key={status}>{status}</option>)}</select><select value={repairSort} onChange={(event) => setRepairSort(event.target.value)} aria-label="Sort repairs"><option value="newest">Newest first</option><option value="oldest">Oldest first</option><option value="ticket">Ticket number</option><option value="customer">Customer A–Z</option><option value="status">Progress low–high</option></select></div><DateRangeFilter period={repairPeriod} setPeriod={setRepairPeriod} from={repairFrom} setFrom={setRepairFrom} to={repairTo} setTo={setRepairTo} label="Filter repairs by intake date"/><Queue title="My assigned repairs" items={mine} action page={minePage} setPage={setMinePage}/><Queue title="My repair history" items={history} page={historyPage} setPage={setHistoryPage}/>
      {detailTicket && <div className="modal-backdrop"><div className="modal card repair-detail-modal"><div className="modal-head"><div><p>REPAIR QUEUE › DETAIL</p><h2>Repair Detail</h2></div><button type="button" onClick={() => setDetailTicket(null)} disabled={savingProgress}>×</button></div>{(role === 'Admin' || role === 'Front Desk') && <div className="row-actions detail-print-actions"><button type="button" className="outline" onClick={() => runDetailPrint('Small ticket', () => printSmallTicket(detailTicket))}>Print Small Ticket</button><button type="button" className="outline" onClick={() => runDetailPrint('Full receipt', () => printFullReceipt(detailTicket))}>Print Full Receipt</button><button type="button" className="outline" onClick={() => runDetailPrint('Both tickets', () => printBoth(detailTicket))}>Print Both</button>{detailPrintStatus && <small>{detailPrintStatus}</small>}</div>}<div className="repair-detail-grid"><section className="repair-detail-panel"><h3>⌕ &nbsp; Device Information</h3><dl className="repair-facts"><dt>Repair ID</dt><dd>{detailTicket.id}</dd><dt>Customer</dt><dd>{detailTicket.customer}</dd><dt>Phone</dt><dd>{detailTicket.phone}</dd><dt>Device</dt><dd>{detailTicket.device}</dd><dt>IMEI / Serial</dt><dd>{detailTicket.imei || 'Not recorded'}</dd><dt>Intake Date</dt><dd>{new Date(detailTicket.createdAt).toLocaleString()}</dd><dt>Assigned To</dt><dd>{detailTicket.tech}</dd><dt>Initial Maintenance Estimate</dt><dd>{money(detailTicket.estimatedCost)}</dd>{detailTicket.finalPrice != null && <><dt>Final Price</dt><dd>{money(detailTicket.finalPrice)}</dd></>}</dl><div className="repair-issue"><strong>ISSUE DESCRIPTION</strong><p>{detailTicket.issue}</p>{detailTicket.condition && <small>Condition: {detailTicket.condition}</small>}</div><div className="repair-used-parts"><strong>PARTS USED</strong>{detailTicket.usedParts?.length > 0 ? detailTicket.usedParts.map((part) => <span key={part.id}>{part.name} <b>× {part.quantity}</b></span>) : <small>No parts added</small>}<div className="repair-cost-breakdown">{historicalPartsCharge > 0 && <span>Previously billed parts <b>{money(historicalPartsCharge)}</b></span>}<span>Maintenance charge <b>{money(detailTicket.finalPrice != null ? detailTicket.serviceCharge : serviceChargeValue)}</b></span><strong>Total <b>{money(detailTicket.finalPrice ?? finalCustomerTotal)}</b></strong></div></div></section><section className="repair-detail-panel update"><h3>✎ &nbsp; {progressValue === 100 ? 'Finalize Price & Ready for Pickup' : detailTicket.status === 'Received' ? 'Review and Take Job' : 'Update Repair Progress'}</h3><form className="repair-update-form" onSubmit={submitRepairProgress}><label>Technician Comments<textarea name="notes" defaultValue={detailTicket.notes} placeholder="Add diagnosis, work completed, or other repair notes…" /></label><label>Maintenance charge including parts (ETB)<input name="serviceCharge" type="number" min="0" step="0.01" value={serviceChargeValue} onChange={(event) => setServiceChargeValue(event.target.value)} required /></label>{detailTicket.status !== 'Received' && <><label>Repair Status<select name="progress" value={progressValue} onChange={(event) => setProgressValue(Number(event.target.value))}>{[[0,'Received'],[25,'Diagnosing'],[50,'Repair Approved'],[75,'In Repair'],[100,'Ready for Pickup']].map(([value,label]) => <option value={value} key={value} disabled={value < detailTicket.progress}>{value}% — {label}</option>)}</select><span className="progress-status-preview">Status: <strong>{progressStatus(progressValue)}</strong></span></label><div className="multi-part-picker"><strong>{progressValue === 100 ? 'Add final parts' : 'Parts used this update'}</strong>{partRows.map((row, index) => <div className="part-usage-fields" key={index}><label>Inventory item<select value={row.id} onChange={(event) => setPartRows((rows) => rows.map((item, itemIndex) => itemIndex === index ? { ...item, id: event.target.value } : item))}><option value="">No item selected</option>{inventory.filter((part) => part.stock > 0 && !detailTicket.usedParts?.some((usedPart) => usedPart.id === part.id) && !partRows.some((selected, selectedIndex) => selectedIndex !== index && selected.id === part.id)).map((part) => <option value={part.id} key={part.id}>{part.name} · {part.stock} available</option>)}</select></label><label>Quantity<input type="number" min="1" step="1" value={row.quantity} onChange={(event) => setPartRows((rows) => rows.map((item, itemIndex) => itemIndex === index ? { ...item, quantity: event.target.value } : item))}/></label>{partRows.length > 1 && <button type="button" className="remove-part" onClick={() => setPartRows((rows) => rows.filter((_, itemIndex) => itemIndex !== index))} aria-label="Remove part">×</button>}</div>)}<button type="button" className="outline add-part" onClick={() => setPartRows((rows) => [...rows, { id: '', quantity: 1 }])}>＋ Add another part</button></div>{progressValue === 100 && <div className="final-price-summary"><span>Parts used are included in the maintenance charge.</span><strong>Final customer total <b>{money(finalCustomerTotal)}</b></strong></div>}</>}<button className="primary" disabled={savingProgress}>{savingProgress ? 'Saving…' : detailTicket.status === 'Received' ? 'Take The Job' : progressValue === 100 ? `Confirm ${money(finalCustomerTotal)} & Ready` : 'Save Update'}</button></form></section></div></div></div>}
    </>;
  }
  return <><PageHead eyebrow="OPERATIONS" title="Repairs queue"><span className="head-count">Received 0% → Diagnosing 25% → Repair Approved 50% → In Repair 75% → Ready for Pickup 100%</span></PageHead><div className="toolbar card list-toolbar"><SearchBox value={search} onChange={setSearch} placeholder="Search ticket, customer, phone, IMEI, device or status" label="Search repairs"/><ResultCount shown={visibleRepairs.length} total={totalRepairs} noun="repair"/><select value={repairStatusFilter} onChange={(event) => setRepairStatusFilter(event.target.value)} aria-label="Filter repairs by status"><option>All</option>{['Received','Diagnosing','Repair Approved','In Repair','Ready for Pickup','Delivered'].map((status) => <option key={status}>{status}</option>)}</select><select value={repairSort} onChange={(event) => setRepairSort(event.target.value)} aria-label="Sort repairs"><option value="newest">Newest first</option><option value="oldest">Oldest first</option><option value="ticket">Ticket number</option><option value="customer">Customer A–Z</option><option value="status">Progress low–high</option></select></div><DateRangeFilter period={repairPeriod} setPeriod={setRepairPeriod} from={repairFrom} setFrom={setRepairFrom} to={repairTo} setTo={setRepairTo} label="Filter repairs by intake date"/><section className="card table-card full-table"><div className="table-scroll"><table><thead><tr><th>Ticket & customer</th><th>Device / issue</th><th>Status</th><th>Assigned to</th><th>Invoice</th><th>Action</th></tr></thead><tbody>{pageRepairs.length ? pageRepairs.map((repair) => <tr key={repair.id}><td><div className="customer"><span className="avatar small">{repair.avatar}</span><div><strong>{repair.id}</strong><small>{repair.customer} · {repair.phone}{repair.isCreditCustomer ? ' · Credit' : ''}</small></div></div></td><td><strong>{repair.device}</strong><small>{repair.issue}</small></td><td><Status value={repair.status}/>{repair.delivery && <small>By {repair.delivery.deliveredBy} · {new Date(repair.delivery.deliveredAt).toLocaleString()}</small>}</td><td>{repair.tech}</td><td><strong>{money(repair.invoiceTotal ?? repair.total)}</strong>{repair.saleId ? <small>{repair.paymentStatus} · Due {money(repair.balanceDue)}</small> : <small>Estimate</small>}</td><td><div className="row-actions">{role === 'Front Desk' && repair.status === 'Received' ? <button className="primary delivery-button" onClick={() => setAssignmentTicket(repair)}>{repair.tech === 'Unassigned' ? 'Assign technician' : 'Reassign'}</button> : role === 'Front Desk' && repair.status === 'Ready for Pickup' ? <button className="primary delivery-button" onClick={() => { setDeliveryTicket(repair); setDeliveryPaymentAmount(canUseCreditForDelivery(repair) ? 0 : repair.balanceDue); setDeliveryPaymentMethod('CASH'); setDeliveryPaymentKey(crypto.randomUUID()); }}>Confirm delivery</button> : <span className="readonly">{repair.status === 'Delivered' ? 'Delivery recorded' : 'Read only'}</span>}{role === 'Front Desk' && <button type="button" className="outline" onClick={() => printTicket(repair)}>⎙ Print</button>}</div></td></tr>) : <tr><td colSpan="6" className="empty">No repairs match the current search and filter.</td></tr>}</tbody></table></div><Pagination page={repairPage} setPage={setRepairPage} totalItems={visibleRepairs.length} pageSize={PAGE_SIZE}/></section>
...
export function InventoryView({ role, permissions = [], parts, categories = [], dashboard = {}, createInventoryItem, updateInventoryItem, deleteInventoryItem, receiveInventoryStock, createCategory, updateCategory, deleteCategory }) {
  const admin = role === 'Admin';
  const canManage = admin || permissions.includes('VIEW_INVENTORY');
  const canManageCategories = canManage || permissions.includes('VIEW_CATEGORIES');
  ...
  const skuMatch = !editing && skuInput.trim() ? parts.find((part) => part.sku.toUpperCase() === skuInput.trim().toUpperCase()) : null;
  const closeForm = () => { if (!saving) { setAdding(false); setEditing(null); } };
  const submit = async (event) => {
    event.preventDefault();
    if (saving || skuMatch) return;
    const formElement = event.currentTarget;
    setSaving(true);
    try {
      const form = Object.fromEntries(new FormData(formElement));
      const saved = editing ? await updateInventoryItem({ ...form, id: editing.id }) : await createInventoryItem(form);
      if (saved) { formElement.reset(); setAdding(false); setEditing(null); setSkuInput(''); setInventoryType(categoriesByName.get(form.category) === 'ACCESSORY' ? 'Accessories' : 'Spare Parts'); }
    } finally { setSaving(false); }
  };
  const typeParts = parts.filter((part) => inventoryType === 'Accessories' ? part.sellAtCheckout : !part.sellAtCheckout);
  ...
  const accessoryCount = parts.filter((part) => part.sellAtCheckout).length;
  const onCategoriesTab = inventoryType === 'Categories';
  return <><PageHead eyebrow="STOCK CONTROL" title="Inventory & parts">{(onCategoriesTab ? canManageCategories : canManage) && <div className="page-actions"><button className="primary" onClick={() => { if (onCategoriesTab) categoryManagerRef.current?.openAdd(); else { setEditing(null); setFormCategory(''); setSkuInput(''); setAdding(true); } }}>{onCategoriesTab ? '＋ Add category' : '＋ Add item'}</button></div>}</PageHead>{admin && <><div className="metric-grid inventory-balance-grid"><Metric icon="□" tone="blue" label="Spare-parts revenue" value={money(dashboard.sparePartsRevenue)} meta="Finalized repair invoices"/><Metric icon="◇" tone="violet" label="Accessories revenue" value={money(dashboard.accessoriesRevenue)} meta="Retail sales and older repair invoices"/><Metric icon="⌁" tone="amber" label="Maintenance revenue" value={money(dashboard.maintenanceRevenue)} meta={`${dashboard.completedJobs || 0} completed jobs`}/><Metric icon="Br" tone="green" label="Total revenue" value={money(dashboard.totalRevenue)} meta="Full finalized invoice value"/></div><section className="card inventory-formula"><strong>Revenue recognition</strong><span>Σ full totals of finalized invoices</span><small>Later customer payments increase cash collected without increasing revenue again.</small></section></>}<div className="inventory-controls"><div className="inventory-tabs card"><button className={inventoryType === 'Spare Parts' ? 'active' : ''} onClick={() => setInventoryType('Spare Parts')}>Spare Parts <b>{parts.length - accessoryCount}</b></button><button className={inventoryType === 'Accessories' ? 'active' : ''} onClick={() => setInventoryType('Accessories')}>Accessories <b>{accessoryCount}</b></button><button className={onCategoriesTab ? 'active' : ''} onClick={() => setInventoryType('Categories')}>Categories <b>{categories.length}</b></button></div>{!onCategoriesTab && <div className="toolbar card list-toolbar inventory-list-toolbar"><SearchBox value={inventorySearch} onChange={setInventorySearch} placeholder="Search name, SKU, category or device" label="Search inventory"/><ResultCount shown={visibleParts.length} total={typeParts.length} noun="item"/><select value={inventorySort} onChange={(event) => setInventorySort(event.target.value)} aria-label="Sort inventory"><option value="name">Name A–Z</option><option value="name-desc">Name Z–A</option><option value="stock-low">Lowest stock</option><option value="stock-high">Highest stock</option><option value="value">Highest value</option><option value="updated">Recently updated</option></select></div>}</div>
    {onCategoriesTab ? <CategoryManager ref={categoryManagerRef} categories={categories} parts={parts} canManage={canManageCategories} createCategory={createCategory} updateCategory={updateCategory} deleteCategory={deleteCategory} /> : <>
      <div className="notice"><span>!</span>{typeParts.filter((part) => part.stock <= part.min).length} {inventoryType.toLowerCase()} are below their minimum stock threshold.</div>
      <section className="card table-card full-table inventory-table"><div className="panel-title"><div><h2>☷ &nbsp; {inventoryType}</h2><p>{visibleParts.length} matching item{visibleParts.length === 1 ? '' : 's'}</p></div></div><div className="table-scroll"><table><thead><tr><th>SKU</th><th>Item Name</th><th>Category</th><th>Qty</th>{admin && <th>Buying Price</th>}<th>Selling Price</th><th>Sales Value</th><th>Description</th><th>Last Updated</th>{canManage && <th>Actions</th>}</tr></thead><tbody>{pageParts.length ? pageParts.map((part) => <tr key={part.id}><td><strong>{part.sku}</strong></td><td><strong>{part.name}</strong></td><td><span className="category-badge">{part.category}</span></td><td><span className={part.stock <= part.min ? 'quantity-badge low' : 'quantity-badge'}>{part.stock}</span></td>{admin && <td>{money(part.buyingPrice)}</td>}<td>{part.sellAtCheckout ? 'Entered at checkout' : money(part.sellingPrice)}</td><td><strong>{part.sellAtCheckout ? '—' : money(part.stock * part.sellingPrice)}</strong></td><td className="description-cell" title={part.description}>{part.description || '—'}</td><td>{new Date(part.updatedAt || part.createdAt).toLocaleDateString()}</td>{canManage && <td><div className="inventory-actions"><button onClick={() => setReceivingPart(part)}>↳ Receive</button><button onClick={() => { setEditing(part); setFormCategory(part.category); setAdding(true); }}>✎ Edit</button><button className="delete" onClick={() => deleteInventoryItem(part.id, part.name)}>♲ Delete</button></div></td>}</tr>) : <tr><td colSpan={8 + (admin ? 1 : 0) + (canManage ? 1 : 0)} className="empty">No inventory items match the current search.</td></tr>}</tbody></table></div><Pagination page={inventoryPage} setPage={setInventoryPage} totalItems={visibleParts.length} pageSize={PAGE_SIZE}/></section>
    </>}
    {receivingPart && <ReceiveStockModal inventory={parts} part={receivingPart} receiveStock={receiveInventoryStock} close={() => setReceivingPart(null)} />}
    {adding && <div className="modal-backdrop"><form className="modal card inventory-modal" onSubmit={submit}><div className="modal-head"><div><p>STOCK CONTROL</p><h2>{editing ? '✎ Edit Inventory Item' : '＋ Add Inventory Item'}</h2></div><button type="button" onClick={closeForm} disabled={saving} aria-label="Close">×</button></div><div className="form-grid"><label>Item Name *<input name="name" key={skuMatch ? `existing-${skuMatch.id}` : editing?.id || 'new'} required autoFocus placeholder="e.g., iPhone Screen" defaultValue={skuMatch ? skuMatch.name : editing?.name || ''} readOnly={Boolean(skuMatch)} /></label><label>SKU {editing ? <input name="sku" defaultValue={editing.sku} required /> : <input name="sku" value={skuInput} onChange={(event) => setSkuInput(event.target.value)} placeholder="Scan SKU or leave blank to generate" />}</label><label>Category *<select name="category" required value={formCategory} onChange={(event) => setFormCategory(event.target.value)}><option value="" disabled>Select category</option><optgroup label="Spare parts">{categories.filter((category) => category.group === 'SPARE_PART').map((category) => <option key={category.id} value={category.name}>{category.name}</option>)}</optgroup><optgroup label="Accessories">{categories.filter((category) => category.group === 'ACCESSORY').map((category) => <option key={category.id} value={category.name}>{category.name}</option>)}</optgroup></select></label><label>Quantity *<input name="quantity" type="number" min="0" step="1" defaultValue={editing?.stock ?? 0} required /></label><label>Buying Price (ETB) *<input name="buyingPrice" type="number" min="0" step="0.01" placeholder="0.00" defaultValue={editing?.buyingPrice ?? ''} required /></label>{categoriesByName.get(formCategory) !== 'ACCESSORY' && <label>Selling Price (ETB) *<input name="sellingPrice" type="number" min="0" step="0.01" placeholder="0.00" defaultValue={editing?.sellingPrice ?? ''} required /></label>}{skuMatch && <p className="sku-match-notice wide">This SKU already exists — <strong>{skuMatch.name}</strong> ({skuMatch.category}). Find it in the list and use its <em>Receive</em> action to add quantity, or <em>Edit</em> to change details.</p>}<label className="wide">Description<textarea name="description" placeholder="Optional details" defaultValue={editing?.description || ''} /></label></div><div className="modal-actions"><button type="button" className="outline" onClick={closeForm} disabled={saving}>Cancel</button><button className="primary" disabled={saving || Boolean(skuMatch)}>{saving ? 'Saving…' : editing ? 'Save Changes' : '＋ Add Item'}</button></div></form></div>}
  </>;
}
...
export function SalesView({ sales, inventory = [], dashboard = {}, recordPayment, createAccessorySale }) {
  ...
  const [saving, setSaving] = useState(false);
  const [showAccessorySale, setShowAccessorySale] = useState(false);
  ...
  const visibleSales = stableSort(
    sales.filter((sale) => (salesStatusFilter === 'All' || sale.status === salesStatusFilter) && withinPeriod(sale.createdAt, salesPeriodBounds) && matchesSearch([sale.id, sale.customer, sale.item, sale.items?.map((item) => item.sku).join(' '), sale.method, sale.status, sale.invoiceStatus, sale.invoiceTotal, sale.amountPaid, sale.balanceDue], salesSearch)),
...
export function ReportsView({ dashboard }) {
  ...
  const rows = [
    ...
    { label: 'Accessories revenue', value: dashboard.accessoriesRevenue, meta: 'Accessory sales and older repair invoices', group: 'Revenue' },
...
export function WebsiteView({ role, permissions = [], banners = [], socialLinks = [], staffProfiles = [], blogPosts = [], createBanner, updateBanner, deleteBanner, reorderBanner, createSocialLink, updateSocialLink, deleteSocialLink, createStaffProfile, updateStaffProfile, deleteStaffProfile, createBlogPost, updateBlogPost, deleteBlogPost, reorderBlogPost }) {
  const hasWebsite = role === 'Admin' || permissions.includes('MANAGE_WEBSITE');
  const canBanners = hasWebsite || permissions.includes('MANAGE_WEBSITE_BANNERS');
  const canSocial = hasWebsite || permissions.includes('MANAGE_WEBSITE_SOCIAL');
  const canShowcase = hasWebsite || permissions.includes('MANAGE_WEBSITE_SHOWCASE');
  const canBlog = hasWebsite || permissions.includes('MANAGE_WEBSITE_BLOG');
  ...
  return <>
    ...
    {canBanners && <section className="card table-card full-table website-section">
    ...
    {canSocial && <section className="card table-card full-table website-section">
    ...
    {canShowcase && <section className="card table-card full-table website-section">
    ...
    {canBlog && <section className="card table-card full-table website-section">
    ...
    {canBanners && addingBanner && <div className="modal-backdrop"><form className="modal card banner-modal" onSubmit={submitBanner}>
    ...
    {canSocial && addingSocial && <div className="modal-backdrop"><form className="modal card" onSubmit={submitSocial}>
    ...
    {canBlog && addingPost && <div className="modal-backdrop"><form className="modal card" onSubmit={submitPost}>
    ...
    {canShowcase && addingStaff && <div className="modal-backdrop"><form className="modal card" onSubmit={submitStaff}>
