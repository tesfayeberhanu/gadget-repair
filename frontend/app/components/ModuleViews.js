...
import { DateRangeFilter, Metric, money, PageHead, ResultCount, RevenueCard, SearchBox, Status } from './SharedUI';
...
export function RepairsView({ repairs, totalRepairs = repairs.length, inventory = [], technicians = [], search, setSearch, role, updateStatus, assignRepair, saveRepairProgress, confirmDelivery }) {
  ...
  const [repairPeriod, setRepairPeriod] = useState('all');
...
export function CustomersView({ role, customers, repairs = [], createCustomer, updateCustomer, startCustomerIntake, focusCustomer, clearFocusCustomer }) {
...
export function AppointmentsView({ appointments, reviewAppointment }) {
  ...
  const [appointmentPeriod, setAppointmentPeriod] = useState('all');
