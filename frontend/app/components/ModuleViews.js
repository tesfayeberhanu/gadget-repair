...
export function TeamView({ team, createStaff, deactivateStaff }) {
  ...
  return <><PageHead eyebrow="ACCESS CONTROL" title="Team & roles"><button className="primary" onClick={() => setAdding((value) => !value)}>＋ Add staff</button></PageHead>
    {adding && <form className="card staff-form" onSubmit={submit}><label>Full name<input name="name" required /></label><label>Email<input name="email" type="email" required /></label><label>Base role<select name="role"><option>Technician</option><option>Front Desk</option></select></label><label>Temporary password<input name="password" type="password" minLength="10" required /></label><fieldset className="permission-picker"><legend>Additional permissions</legend>{[['VIEW_REPORTS','Reports'],['VIEW_CUSTOMERS','Customer data'],['MANAGE_POS','Point of sale'],['VIEW_INVENTORY','Inventory']].map(([value,label]) => <label key={value}><input type="checkbox" name="permissions" value={value}/>{label}</label>)}</fieldset><div><button type="button" className="outline" onClick={() => setAdding(false)}>Cancel</button><button className="primary">Create account</button></div></form>}
    <div className="team-grid">{team.map(({ id, email, name, role, permissions = [], description }) => <article className="card team-member" key={id}><span className="avatar large">{name.split(' ').map((part) => part[0]).join('')}</span><div><h3>{name}</h3><p>{email}</p><small>{description}</small>{permissions.length > 0 && <div className="permission-tags">{permissions.map((permission) => <i key={permission}>{permission.replaceAll('_', ' ').toLowerCase()}</i>)}</div>}</div><span className="role-badge">{role}</span>{role !== 'Admin' && <button className="staff-remove" onClick={() => deactivateStaff(id, name)}>Deactivate</button>}</article>)}</div></>;
}
