...
const CategoryManager = forwardRef(function CategoryManager({ categories, parts, canManage = true, createCategory, updateCategory, deleteCategory }, ref) {
  ...
  return <section className="card table-card full-table category-manager-inline">
    ...
    <div className="category-list">{sorted.map((category) => <div className="category-row" key={category.id}>
      ...
      {canManage && <div className="category-row-actions"><button type="button" onClick={() => startEdit(category)}>✎ Edit</button><button type="button" className="delete" onClick={() => deleteCategory(category.id, category.name)}>♲ Delete</button></div>}
    </div>)}</div>
    {canManage && formOpen && <form className="category-form" onSubmit={submit}>
