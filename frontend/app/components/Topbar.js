export default function Topbar({ role, user, search, setSearch, openIntake, notificationCount = 0, openNotifications }) {
  return <header className="topbar">
    ...
    <div className="top-actions">
      ...
      <button className="icon-button notification-bell" aria-label={`${notificationCount} available repair notifications`} title="Available repairs" onClick={openNotifications}><span aria-hidden="true">🔔</span>{notificationCount > 0 && <i>{notificationCount > 99 ? '99+' : notificationCount}</i>}</button>
