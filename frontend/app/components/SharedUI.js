...
export function SalesCard({ sales }) { return <section className="card panel"><div className="panel-title"><div><h2>Recent sales</h2><p>Latest completed transactions</p></div></div><div className="mini-list">{sales.map((sale) => <div key={sale.id}><span className="sale-icon">▤</span><div><strong>{sale.customer}</strong><small>{sale.item}</small></div><b>{money(sale.amount)}</b></div>)}</div></section>; }
