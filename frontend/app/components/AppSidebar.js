const icons = { Overview: '⌂', Repairs: '⌁', Inventory: '□', 'Point of Sale': '◇', Customers: '♙', Reports: '↗', Team: '♧', Settings: '⚙', 'New Intake': '＋', Appointments: '◷' };
...
export default function AppSidebar({ role, user, active, navigation, repairs, setActive, openIntake, logout }) {
  return <aside className="sidebar">
    ...
    <nav className="side-nav"><p>WORKSPACE</p>{navigation.map((item) => <button key={item} className={active === item ? 'active' : ''} aria-label={item} title={item} onClick={() => item === 'New Intake' ? openIntake() : setActive(item)}><span className="nav-icon">{icons[item]}</span><span className="nav-label">{item}</span>{item === 'Repairs' && <b>{repairs.filter((repair) => !['Ready for Pickup', 'Delivered'].includes(repair.status)).length}</b>}</button>)}</nav>
