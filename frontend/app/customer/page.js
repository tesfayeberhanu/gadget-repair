...
export default function CustomerPage() {
  ...
  return <div className="customer-site">
    <header className="customer-nav"><a href="#home" className="customer-brand"><img src="/ifixlab251-logo.png" alt="iFixLab251"/><span>iFixLab<span>251</span></span></a><nav><a href="#track">Track repair</a><a href="#appointment">Appointment</a><a href="#tips">Repair tips</a><a className="staff-link" href="/">Staff sign in</a></nav></header>
    <main>
      ...
      <section className="customer-section track-section" id="track"><div className="section-copy"><p className="customer-kicker">LIVE REPAIR UPDATES</p><h2>Track your phone status</h2><p>Enter the ticket number from your receipt and the same phone number used at check-in.</p></div><div className="customer-panel"><form className="public-form inline-form" onSubmit={track}><label>Ticket number<input name="ticket" placeholder="REP-2026-0142" required /></label><label>Phone number<input name="phone" type="tel" placeholder="+211 ..." required /></label><button className="customer-primary">Check status</button></form>{trackError && <div className="public-error">{trackError}</div>}{tracking && <div className="tracking-result"><div><small>{tracking.ticketNumber}</small><h3>{tracking.device}</h3><p>Assigned: {tracking.technician}</p></div><div className="status-steps">{statuses.map((status, index) => { const current = statuses.indexOf(tracking.status); return <span className={index <= current ? 'done' : ''} key={status}><i>{index < current ? '✓' : index + 1}</i>{status}</span>; })}</div></div>}</div></section>

      <section className="customer-section appointment-section" id="appointment"><div className="appointment-copy"><p className="customer-kicker">PLAN YOUR VISIT</p><h2>Request an appointment</h2><p>Tell us what is happening and choose a convenient time. The front desk will confirm availability.</p><ul><li>Faster check-in</li><li>No account required</li><li>Confirmation reference provided</li></ul></div><form className="customer-panel public-form appointment-form" onSubmit={book}><label>Your name<input name="customerName" autoComplete="name" required /></label><label>Phone number<input name="customerPhone" type="tel" autoComplete="tel" required /></label><label>Device<input name="device" placeholder="e.g. Samsung S24" required /></label><label>Preferred date & time<input name="preferredDate" type="datetime-local" min={today} required /></label><label className="wide">What needs attention?<textarea name="issue" placeholder="Briefly describe the problem" required /></label>{appointmentError && <div className="public-error wide">{appointmentError}</div>}{appointment && <div className="public-success wide">Request received. Reference: <strong>{appointment.reference}</strong></div>}<button className="customer-primary wide">Request appointment</button></form></section>
    ...
    <footer className="customer-footer"><div className="customer-brand"><img src="/ifixlab251-logo.png" alt=""/><span>iFixLab<span>251</span></span></div><p>Device repair, clear communication, dependable care.</p><a href="/">Staff portal</a></footer>
  </div>;
}
