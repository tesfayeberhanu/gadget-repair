...
export default function CustomerPage() {
  ...
  return <div className="customer-site">
    ...
    <main>
      ...
      <section className="customer-section track-section" id="track"><div className="section-copy"><p className="customer-kicker">LIVE REPAIR UPDATES</p><h2>Track your phone status</h2><p>Enter the ticket number from your receipt and the same phone number used at check-in.</p></div><div className="customer-panel"><form className="public-form inline-form" onSubmit={track}><label>Ticket number<input name="ticket" placeholder="REP-2026-0142" required /></label><label>Phone number<input name="phone" type="tel" placeholder="0912345678 or +251912345678" pattern="(?:09\d{8}|\+2519\d{8})" title="Use 09XXXXXXXX or +2519XXXXXXXX" required /></label><button className="customer-primary">Check status</button></form>{trackError && <div className="public-error">{trackError}</div>}{tracking && <div className="tracking-result"><div><small>{tracking.ticketNumber}</small><h3>{tracking.device}</h3><p>Assigned: {tracking.technician}</p></div><div className="status-steps">{statuses.map((status, index) => { const current = statuses.indexOf(tracking.status); return <span className={index <= current ? 'done' : ''} key={status}><i>{index < current ? '✓' : index + 1}</i>{status}</span>; })}</div></div>}</div></section>
